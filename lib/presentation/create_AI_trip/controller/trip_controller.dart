  ...
  Future<void> getTripByUserId(int userId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Enhance trip data with city details
        for (var trip in trips) {
          if (trip.tripDestinations.isNotEmpty) {
            final destinationId = trip.tripDestinations.first.destinationId;

            // Fetch destination details to get city_id
            final cityId = await _getCityIdByDestinationId(destinationId);

            // Map city_id to CityData
            if (cityId != null) {
              final cityData = cityList.firstWhereOrNull((city) => city.id == cityId);
              if (cityData != null) {
                trip.imageUrl = cityData.imageUrl;
                trip.cityName = cityData.name;
              }
            }
          }
        }
