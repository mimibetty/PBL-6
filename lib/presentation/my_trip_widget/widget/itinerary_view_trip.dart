...
class _ItineraryViewTripState extends State<ItineraryViewTrip>
  ...
  void _initializeData() {
    ...
    // Lọc bỏ day_0
    dailyScheduleKeys = widget.jsonResponse['daily_schedule']
        ?.keys
        .where((key) => _getDayNumber(key) > 0) // Chỉ lấy những ngày lớn hơn 0
        .toList() ?? [];
    ...
    allDestinations = {
      for (var destination in widget.hotelList + widget.restaurantList + widget.thingsToDoList)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // TabBar hiển thị danh sách ngày và tab "Places to Stay"
          TabBar(
            ...
            tabs: [
              ...
              for (var dayKey in dailyScheduleKeys) Tab(text: _formatDayLabel(dayKey)),
            ],
          ),
          Expanded(
            child: TabBarView(
              ...
              children: [
                ...
                // Các tab hiển thị lịch trình hàng ngày
                for (var dayKey in dailyScheduleKeys)
                  DailyScheduleTab(
                    ...
                    destinations: (dailySchedule[dayKey] as List<dynamic>?)
                        ...
                        .whereType<TravelDestination>() // Lọc null và chỉ giữ các đối tượng hợp lệ
                        .toList() ??
                        [],
...
class DailyScheduleTab extends StatelessWidget {
  final String dayKey;
  final List<TravelDestination> destinations; // Danh sách đối tượng đích thay vì chỉ ID
