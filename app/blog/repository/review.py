...
def get_reviews_of_tour_by_tourId(tour_id: int, db: Session):
    try:
        reviews = db.query(models.Review).filter(models.Review.tour_id == tour_id).all()  # Chờ truy vấn
        return reviews
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Error retrieving reviews: {str(e)}")
...
def get_reviews_of_user_in_1_tour_by_userId_and_tourID(tour_id: int, user_id: int, db: Session):
    try:
        # Truy vấn chỉ các review có tour_id và user_id tương ứng
        reviews = db.query(models.Review).filter(
            models.Review.tour_id == tour_id,
            models.Review.user_id == user_id
        ).all()  # Lấy tất cả các review
