...
def create_city_by_userid(request: schemas.City, user_id: int, db: Session):
    try:
...
def get_city_by_userid(user_id: int, db: Session):
    try:
        
        city = db.query(models.City).filter(models.City.user_id == user_id).first()  # Chờ truy vấn
        print(city)
        
        if not city:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=f"City with the user_id {user_id} is not available")
        return city
    except Exception as e:
...
def update_city_by_userid(user_id: int, request: schemas.City, db: Session):
    try:
        city = db.query(models.City).filter(models.City.user_id == user_id).first()  # Chờ truy vấn
        if not city:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def delete_city_by_userid(user_id: int, db: Session):
    try:
        city = db.query(models.City).filter(models.City.user_id == user_id).first()  # Chờ truy vấn
        if not city:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
