<template>
  <div class="carousel container">
    <VSlickCarousel v-bind="settings">
      <div class="tour-item p-2" v-for="(tour, index) in tours" :key="index" @click="navigateToDetailTour(tour.id)">
        <div class="tour-img">
          <img v-if="tour.destinations.find(dest => dest.images && dest.images.length)?.images[0]"
            :src="tour.destinations.find(dest => dest.images && dest.images.length)?.images[0].url" :alt="tour.id"
            class="w-100">
          ...
          <div class="tour-info d-flex border-start-0
            ...
            <small class="flex-fill text-center border-end py-2">
              <svg class="fa me-2" width="30px" height="30px" viewBox="0 0 24 24" fill="#caf0f8"
            ...
            <small class="flex-fill text-center py-2">
              <svg class="fa me-2" fill="#caf0f8" width="30px" height="30px" viewBox="0 0 32 32"
        ...
        <div class="tour-content bg-light">
          <div class="p-4 pb-0">
            <h5 class="tour-location text-truncate" style="margin-bottom: 15px;">{{ tour.name }}</h5>
            <div class="mb-3 rating">
              <small class='text-uppercase number-rating'>{{ tour.rating }}</small>
              <div class="col">
                <img class="fa icon-rating" v-for="star in generateStars(tour.rating)" :src="star" />
              </div>
            </div>
