...
class TravelPlanner:
    def __init__(self, locations, distances, centers, min_points_per_cluster=4):
        """
        locations: list of tuples (name, lat, long) - tên và tọa độ các địa điểm
        distances: matrix khoảng cách giữa các địa điểm
...
def test_travel_planner():
    """Hàm test với dữ liệu mẫu"""
    # Danh sách địa điểm: (tên, vĩ độ, kinh độ)
    locations = [
        ("Hồ Hoàn Kiếm", 21.0285, 105.8542),      # 0
        ("Văn Miếu", 21.0293, 105.8355),          # 1
        ("Lăng Bác", 21.0367, 105.8346),          # 2
        ("Chùa Một Cột", 21.0358, 105.8333),      # 3
        ("Hoàng Thành Thăng Long", 21.0359, 105.8422),  # 4
        ("Nhà Thờ Lớn", 21.0286, 105.8493),       # 5
        ("Bảo tàng Lịch sử", 21.0369, 105.8579),  # 6
        ("Phố cổ Hà Nội", 21.0358, 105.8512),     # 7
        ("Nhà hát lớn", 21.0242, 105.8573),       # 8
        ("Chợ Đồng Xuân", 21.0380, 105.8490)      # 9
    ]
    
    # Ma trận khoảng cách (km) - giả lập
    distances = np.array([
        [0.0, 2.1, 3.2, 3.0, 2.8, 0.5, 2.7, 1.8, 1.2, 2.5],
        [2.1, 0.0, 1.5, 1.3, 1.1, 1.8, 2.4, 2.0, 2.3, 1.9],
        [3.2, 1.5, 0.0, 0.3, 0.7, 2.9, 1.6, 1.4, 3.1, 1.2],
        [3.0, 1.3, 0.3, 0.0, 0.9, 2.7, 1.8, 1.6, 2.9, 1.4],
        [2.8, 1.1, 0.7, 0.9, 0.0, 2.5, 1.2, 1.0, 2.7, 0.8],
        [0.5, 1.8, 2.9, 2.7, 2.5, 0.0, 2.4, 1.5, 0.9, 2.2],
        [2.7, 2.4, 1.6, 1.8, 1.2, 2.4, 0.0, 0.8, 2.1, 0.6],
        [1.8, 2.0, 1.4, 1.6, 1.0, 1.5, 0.8, 0.0, 1.3, 0.7],
        [1.2, 2.3, 3.1, 2.9, 2.7, 0.9, 2.1, 1.3, 0.0, 2.0],
        [2.5, 1.9, 1.2, 1.4, 0.8, 2.2, 0.6, 0.7, 2.0, 0.0]
    ])
    
    # Chọn các điểm làm trung tâm (index của các địa điểm)
    centers = [0, 4]  # Hồ Hoàn Kiếm và Hoàng Thành Thăng Long làm trung tâm
    
    # Khởi tạo planner
    planner = TravelPlanner(locations, distances, centers, min_points_per_cluster=4)
